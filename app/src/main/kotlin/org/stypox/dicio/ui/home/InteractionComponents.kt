...
fun InteractionList(
    ...
    fun LazyListScope.countedItem(
        ...
        if (canBeAnchor) lastAnchorIndex = totalItemCount
    ...
    LaunchedEffect(key1 = newHasPending, key2 = newInteractionCount) {
        if ((!prevHasPending && newHasPending) || newInteractionCount > prevInteractionCount) {
            scope.launch { listState.scrollToItem(lastAnchorIndex) }
        }
...
fun ConfirmedQuestionCard(
    ...
    MessageCard(
        ...
        Row(
            ...
            Icon(imageVector = Icons.Default.Person, contentDescription = null)
            Text(text = userInput, style = MaterialTheme.typography.titleMedium)
        }
    }
}
...
fun MessageCard(
    ...
    val modifier = Modifier.fillMaxWidth().padding(horizontal = 8.dp)
    if (onClick == null) {
